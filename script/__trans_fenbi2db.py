...
#DATA_FILE_1 = 'F:/StockHisdata/SCL8/202010/20201009/scl8_20201009.csv'
        ...
        for line in fh: 
            ...
            try:   
                ...
                if (hhmm == 859 or hhmm == 2059): #front line filter unnecessary data
                    ...
                    continue
                #print("{0} hour:{1} minute:{2}".format(sys._getframe().f_lineno, hour, minute))
                cur_price = float(match1.group(5)) 
                buy1_p = float(match1.group(25))
                buy1_v = int(match1.group(26))
                sell1_p = float(match1.group(27))
                sell1_v = int(match1.group(28))
                ...
                if hhmm == pre_hhmm:
                    ...
                    if not IsTradeWindowStartHhmm(hhmm): # 当前非时段起点
                        ...
                        if not IsInSameTradeWindow(hhmm, pre_hhmm): #跨时段,异常
                            temp_str = "Save1mDataFromFile {0} switch trade win but lack too much date from {1} to {2} ".format(date, pre_hhmm, hhmm)
                            print(temp_str)
                            self.write_log(temp_str) 
                            return False
                        else: #与前数据 是相同时段
                            #print("line 403")
                            t_obj = dt.datetime(date//10000, date%10000//100, date%100, pre_hhmm//100, pre_hhmm%100)
                            t_obj1 = dt.datetime(date//10000, date%10000//100, date%100, hhmm//100, hhmm%100)
                            distance = t_obj1 - t_obj
                            is_first_fake = True
                            if distance.seconds > 60: #中间缺少数据 
                                #fake all lack k and insert 
                                while t_obj < t_obj1: 
                                    tag_hhmm = NextHhmm(date, t_obj.hour*100 + t_obj.minute)
                        ...
                        if not IsTradeWindowEndHhmm(pre_hhmm): #前面不是时段终点,则缺少了数据 
                            ...
                            if w_i >= 0: #if  < 0, may be time 859
                                ...
                                t_obj = dt.datetime(date//10000, date%10000//100, date%100, pre_hhmm//100, pre_hhmm%100)
                                t_obj1 = dt.datetime(date//10000, date%10000//100, date%100, temp_hhmm//100, temp_hhmm%100)
                                is_first_fake = True
                ...
                temp_str = "Save1mDataFromFile {0} line {1} esception {2}".format(file_full_path, line_count, e)
                print(temp_str)
                self.write_log(temp_str)
                return False  
            line_count += 1
...
def main():
    ...
    date = 20201009  #20201012  #20201009
    ...
    #DATA_FILE_PATH = 'F:/StockHisdata/SCL8/201901'
    #DATA_FILE_PATH = 'F:/StockHisdata/SCL8/201902'
    code = "SC1904"
    
    #DATA_FILE_PATH = 'F:/StockHisdata/SCL8/201903'
    DATA_FILE_PATH = 'F:/StockHisdata/SCL8/201904'
    code = "SC1905"
    ...
    if 0:
        ...
        return
    if 0:   
        file_full_path = "F:/StockHisdata/SCL8/201901/20190103/{0}_20190103.csv".format(code)
        ret = obj.ChecFenbiDataFromFile(file_full_path, code)
        if ret:
            obj.ClearRelDbData(code, 20190103)
            ret = obj.Save1mDataFromFile(file_full_path, code) 
        #return
    if 0:
        ret = obj.Save5mDataFrom1mData(code, 20190103)
        return
